...
# 학습용 말뭉치 데이터를 불러옴
corpus = read_file('c:/workspace3/FoodShop/data/ner_train.txt')
...
for t in corpus:
    ...
    for w in t:
        tagged_sentence.append((w[1], w[3]))        #      0    1   2   3
        sentence.append(w[1])                       #      3    25  SN  B_TI
        bio_tag.append(w[3])
...
# 토크나이저 정의
tag_tokenizer = preprocessing.text.Tokenizer(lower=False)   # 태그 정보는 lower=False : 소문자로 변환X
...
model.save('c:/workspace3/FoodShop/model/ner_model.h5')

from tensorflow import keras

model = keras.models.load_model('c:/workspace3/FoodShop/model/ner_model.h5')
